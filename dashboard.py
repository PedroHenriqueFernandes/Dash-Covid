...
fig = px.choropleth_mapbox(df_states_, locations="estado", color="casosNovos", geojson=brasil_states, 
    center={"lat": -16.85, "lon": -47.78}, zoom=4,
    color_continuous_scale="Redor", opacity=0.4, hover_data=
    {"casosNovos":True, "casosAcumulado":True, "obitosNovos":True, "estado":True},)
...
app.layout = dbc.Container(
    dbc.Row([
        dbc.Col([
         html.Div([
                html.H5("Evolução COVID-19", style={"color":"#fff"}),
...
def plot_line_graph(plot_type, location):
    ...
    if plot_type in bar_plots:
        fig2.add_trace(go.Bar(x=df_data_on_location["data"], y=df_data_on_location[plot_type], name=plot_type))
    else:
        fig2.add_trace(go.Scatter(x=df_data_on_location["data"], y=df_data_on_location[plot_type], name=plot_type))
